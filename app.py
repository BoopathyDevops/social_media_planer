...
def main():
    ...
    with st.sidebar:
        st.header("Campaign Settings")
        brand = st.text_input("🏢 Brand name")
        industry = st.text_input("🏭 Industry (e.g., Tech, Fashion, Food)")
        goals = st.text_area("🎯 Campaign goals (e.g., Brand awareness, Increase engagement, Boost sales)", height=100)
        audience = st.text_input("👥 Target audience (e.g., Students, Professionals, Business owners, General audience)")
        weeks = st.slider("📅 Number of weeks", 1, 4, 2)
        platforms = st.multiselect(
            "📱 Platforms",
            ["instagram", "twitter", "linkedin", "facebook"],
